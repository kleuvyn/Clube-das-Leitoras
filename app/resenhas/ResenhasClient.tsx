...
function normalizeMonthName(monthName: string): number | null {
  ...
  for (const [chaveNomeMes, valorNumMes] of Object.entries(MESES_PT)) {
    const normalizedName = chaveNomeMes
      .normalize('NFD')
      .replace(/\p{Diacritic}/gu, '')
      .toLowerCase();
    if (normalizedName === cleaned || normalizedName.startsWith(cleaned) || cleaned.startsWith(normalizedName)) {
      return Number(valorNumMes);
    }
  }
...
function extrairAnoMes(r: Resenha): { ano: number; mes: number } {
  if (r.publishedAt) {
    const parts = r.publishedAt.split('/');
    if (parts.length >= 2) {
      const ano = parseInt(parts[1], 10);
      const mes = normalizeMonthName(parts[0]) ?? 1;
      if (!Number.isNaN(ano)) return { ano, mes };
    }
  }
...
function labelMes(r: Resenha): string {
  ...
  const d = parseDateValue(null) ?? normalizeDateValue(r.createdAt)
  return formatMonthYear(d)
}

// use extracted ResenhaComments component instead

export default function ResenhasClient({ initialResenhas }: { initialResenhas: Resenha[] }) {
  const [resenhas, setResenhas] = useState<Resenha[]>(initialResenhas);
  const [loading, setLoading] = useState(false);
  const [abertas, setAbertas] = useState<Set<string>>(new Set());
  const [anosExpandidos, setAnosExpandidos] = useState<Record<number, boolean>>({});
  ...
  useEffect(() => {
    async function loadFull() {
      try {
        const res = await fetch('/api/resenhas?limit=100');
        if (res.ok) {
          const data = await res.json();
          // the api returns `{ data: rows }` when using pagination query params
          if (data && data.data) {
              setResenhas(data.data.map((r: any) => ({
                ...r,
                createdAt: r.createdAt instanceof Date ? r.createdAt.toISOString() : String(r.createdAt ?? ''),
              })));
          } else if (Array.isArray(data)) {
             setResenhas(data.map((r: any) => ({
                ...r,
                createdAt: r.createdAt instanceof Date ? r.createdAt.toISOString() : String(r.createdAt ?? ''),
              })));
          }
          ...
          console.error("Erro ao carregar imagens completas", err)
      }
    }
    loadFull();
  }, []);
    ...
    >
      ...
      <main className="max-w-4xl mx-auto px-6 relative z-10 mt-24 space-y-32">
        ...
        {loading ? (
          ...
          anos.map(ano => {
            const isExpanded = anosExpandidos[ano] !== undefined ? anosExpandidos[ano] : ano === anos[0];
            return (
              <section key={ano}>
                
                <div className="flex items-end gap-6 mb-20 select-none">
                  <span
                    className="text-[100px] md:text-[140px] font-light tracking-tighter leading-none cursor-pointer"
                  ...
                  <div className="flex-1 mb-6">
                    ...
                    <div className="flex items-center justify-between mt-3">
                      ...
                      <button 
                        onClick={() => toggleAno(ano)} 
                        className="text-[10px] font-bold tracking-[0.3em] uppercase opacity-40 hover:opacity-80 transition-opacity text-[#2C3E50] border border-black/10 px-3 py-1 rounded-full cursor-pointer"
                      >
