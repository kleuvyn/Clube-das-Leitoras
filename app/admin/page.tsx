...
export default async function DashboardPage() {
  ...
  const dashboardCards = [
    ...
    { label: 'Lojinha', value: 'CMS', color: 'border-l-[#5B7C99]', icon: ShoppingBag, href: '/admin/lojinha', desc: 'Produtos e estoque' },
    ...
    <div className="min-h-screen p-6 md:p-12 font-alice bg-[#FDFCFB] text-[#1A1A1A]">
      <div className="p-6 md:p-12 font-alice bg-white text-[#1A1A1A]">
        <header className="mb-16 flex flex-col md:flex-row justify-between items-start md:items-end gap-8 border-b border-[#E5E1DA] pb-12">
          <div>
            <div className="flex items-center gap-3 mb-4 text-rosa-gabi">
              <Coffee size={14} />
        ...
        <section className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {dashboardCards.map((c) => (
            <Link key={c.label} href={c.href} className="group">
              <div className={`h-full bg-white rounded-4xl p-8 border border-[#E5E1DA] border-l-4 ${c.color} hover:shadow-lg transition-all duration-300 hover:-translate-y-1`}>
                <div className="flex justify-between items-start mb-6">
                  <div className="space-y-1">
                    <p className="text-[9px] font-mono font-bold text-[#1A1A1A] uppercase tracking-[0.3em]">{c.label}</p>
                    <p className="text-[10px] italic text-rosa-gabi">{c.desc}</p>
                  </div>
                  <c.icon size={18} className="text-rosa-gabi group-hover:text-[#1A1A1A] transition-all" />
                </div>
