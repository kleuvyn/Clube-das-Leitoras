...
const ALL_ALLOWED_TYPES = [...ALLOWED_IMAGE_TYPES, ...ALLOWED_DOCUMENT_TYPES, ...ALLOWED_AUDIO_TYPES];
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (!isImage && !isAudio && !isDocument) {
      return NextResponse.json({ error: 'Tipo não permitido. Use JPG/PNG/WebP para imagens, PDF para documentos ou MP3/M4A/WAV para áudio.' }, { status: 400 });
    }
    ...
    if (!isImage) {
      ...
      try {
        ...
        // Permitir upload público apenas para imagens de formulários de solicitação.
      }
    }

    const maxSize = isAudio ? MAX_AUDIO_SIZE : isDocument ? MAX_DOCUMENT_SIZE : MAX_IMAGE_SIZE;
    if (file.size > maxSize) {
      return NextResponse.json({ error: isAudio ? 'Áudio muito grande. Máximo 150 MB.' : isDocument ? 'PDF muito grande. Máximo 10 MB.' : 'Imagem muito grande. Máximo 5 MB.' }, { status: 400 });
    }
